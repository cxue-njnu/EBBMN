...
def computeSliceMatrix(
    ...
    end_t = min(
        start_t + int(FREQUENCY * clip_len), int(FREQUENCY * curr_seizure_time[1])
    )
...
class SeizureDataset(Dataset):
    def __init__(
        ...
        file_marker_dir = os.path.join(FILEMARKER_DIR, split + "_data.csv")
        with open(file_marker_dir, "r") as f:
            f_str = f.readlines()[1:]  # 跳过第一行
    ...
    def _get_indiv_graphs(self, eeg_clip, swap_nodes=None):
        ...
        if swap_nodes is not None:
            for node_pair in swap_nodes:
                node_name0 = [
                    key for key, val in sensor_id_to_ind.items() if val == node_pair[0]
                ][0]
                node_name1 = [
                    key for key, val in sensor_id_to_ind.items() if val == node_pair[1]
                ][0]
    ...
    def __getitem__(self, idx):
        ...
        """
        edf_fn, seizure_class = self.file_tuples[idx]
        # 只返回两个，第一个就是文件名
        ...
        if self.preproc_dir is None:
            ...
            with h5py.File(
                os.path.join(self.preproc_dir, edf_fn),
                "r",
            ) as hf:
                eeg_clip = hf["clip"][()]
